...
### M2 — 人机交互 + 遥测面板 (已完成 v0.1.2)
| 文件 | 内容 |
|---|---|
| src/ui/main_window.py | 统一单页 QSplitter 布局, 连接/指令/遥测同屏显示 |
| src/ui/panels/comm_panel.py | 连接控制 + 报文滚动显示 (方向色标 + HEX 文本) |
| src/ui/panels/command_panel.py | 指令面板 (TM1/TM2/查询, 单次+独立轮询) |
| src/ui/panels/telemetry_panel.py | TM1/TM2/查询分页实时刷新 |
| src/ui/components/telemetry_table.py | 8 列 (时间/参数ID/名称/工程量/原始值/源码值/单位/状态) |
...
- 每行显示当前帧接收时间戳 (HH:MM:SS)
- 工程量/原始值/源码值变化时浅绿高亮闪烁 2 秒
- 异常值红色标记 (status=warning)
- 顶部全局最后更新时戳

**性能优化** (2026-06-09):
- QTableWidgetItem 复用: 同一单元格只创建一次 item, 后续仅 setText()
- 批量更新: setUpdatesEnabled(False) 包裹整个循环, 避免逐次重绘
- 单一定时器清除高亮: 替代每变化一个值就创建一个 QTimer 的旧方案

**交付**: 统一单页布局无需切换标签页。连接与指令分离, 手动触发 TM1/TM2 独立
单次 + 同时轮询, 查询包单次查询。TCP keepalive 防止串口盒空闲断连。
**动态导入**: 文件菜单 → 导入遥测定义, 支持任意数量遥测包类型的 Excel 模板。

### M2.5 — 立即遥控指令面板 (新增 v0.1.2)
| 文件 | 内容 |
|---|---|
| src/ui/panels/remote_command_panel.py | 立即遥控指令面板 (Excel 配置驱动, 搜索/分组/参数编辑/帧预览/发送确认) |
| src/protocol/remote_command_loader.py | 立即遥控指令配置加载器 (106 条指令, 12 组, 0.3s 加载) |

**功能**:
- 自动加载 立即遥控指令配置表.xlsx (106 条指令)
- 模糊搜索: 按指令名称/代号实时过滤
- 分组折叠: 模式切换/电磁阀控制/参数设置等 12 组, 可展开/收起
- 参数编辑: 选中指令后参数值可修改 (HEX 空格分隔), 格式错误红色提示
- 指令组包: 按 Excel 配置表数据域长度直接组包, 预览完整帧
- 指令发送: 发送前弹出确认对话框 (含指令名/码/参数/帧), 确认后发送

**布局变更**: 报文日志移至遥测显示下方, 腾出左下位置给遥控指令面板。
