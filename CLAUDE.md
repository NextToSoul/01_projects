...
- 修改代码时遵循影响域最小原则，只改动与需求直接相关的文件、函数、数据和行为。
- 禁止顺手修改无关的代码风格、注释、格式、未使用的导入、或任何未经用户要求的重构。
- 添加新抽象层（函数、类、配置）之前先判断：是否真正消除了重复或复杂度，是否有既有的本地模式可沿用。
- 测试覆盖范围与改动风险匹配：窄改动只加窄测试，不影响已有行为的不增测。

## Chinese Conversation

- 与用户交流默认使用中文。
- 代码中的标识符、注释、日志按项目现有风格处理（不强制中文）。
- 术语、命令、代码引用等保留原文不翻译。
## Avoid Hardcoding

- 编码时优先考虑可配置、可扩展的设计，避免将可能变化的参数、类型定义、映射关系等写死在代码中。
- 配置信息（如指令码、APID、类型映射表、显示标签等）优先放在外部 YAML/配置文件中，代码通过加载机制动态读取。
- 界面元素的生成优先由数据驱动（遍历配置列表动态创建），而不是在代码中逐一写死每个按钮、页签或行。
- 文件路径、网络地址、端口等应使用配置文件或常量定义，不散落在代码各处。
- 硬编码与最小影响域原则不冲突：当改动涉及少量且稳定的硬编码时，若引入抽象层反而增加复杂度，则不强行抽取；但若硬编码明显限制了扩展性（如"新增一个类型就要改 4 个文件"），则优先做动态化改造。